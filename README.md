...
resultsAC <- identifyAC(results, condition = condition, th.pvalue = 0.01, th.mean = 2)
## [START] - computing ACs
## Sampled used :
...
```
## <a name="stat_function_identifyDEC"/> 5.2 Computation of differentially enriched clusters
...
resultsDEC <- identifyDEC(results,conditions = conditions, th.pvalue = 0.01, th.fc = 3)
...
## see @result slot to get further informations...
```
## <a name="stat_function_identifyCC"/> 5.3 Computation of correlated clusters
...
resultsCC <- identifyCC(results, variable = variable, th.pvalue = 0.01, th.correlation = 0.7)
## [START] - computing CCs
## [END] - computing CCs
print(resultsCC)
...
## see @result slot to get further informations...
```
## <a name="stat_function_classifyPhenoProfiles"/> 5.4 Classification of clusters based on their phenotype profiles
...
## <a name="stat_function_classifyEnrichmentProfiles"/> 5.5 Classification of clusters based on enrichment profiles
 ...
 * "both" value will show marker median expressions for each selected samples together with the mean of median maker expressionsons for all selected samples 
 
In order to restrain vizualisation to specific markers and cluster, it is possible to provide a vector of marker names to `markers` parameter or in the same way a vector of clusters to `clusters` parameter.
...
```r
samples <- c(TRUE,TRUE,FALSE,TRUE,TRUE)
